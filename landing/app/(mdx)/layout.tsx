  ...
  return (
    <div className="grid grid-cols-4 md:grid-cols-5 gap-4 px-2">
      ...
      <div className="col-span-4 space-y-3">
        ...
        <div className="prose md:prose-lg mb-10 dark:prose-invert prose-zinc">{children}</div>
      </div>
    </div>
  );
}

const BreadCrumb = ({ paths, page, i = 0 }: { page: Page; paths: string[]; i?: number }) => {
    ...
    <>
      ...
      {next && (
        <>
          ...
          <BreadCrumb page={next} paths={paths} i={i + 1} />
        </>
      )}
    </>
  );
...
const MenuItem = ({ id, title, pages, parents }: Page & { parents: string }) => {
    ...
    <div className="flex flex-col space-y-1">
      <div
        className={`w-full bg-opacity-10 rounded-md duration-150 flex justify-between items-center px-2 ${
          isSelected ? "bg-primary text-primary" : "hover:bg-base-content hover:bg-opacity-10"
        }`}
      >
        <Link className="h-full w-full py-1 opacity-90" href={url} onClick={() => isSelected && setOpen(!open)}>
          {title}
        </Link>
        {pages && (
          <MdOutlineArrowForwardIos onClick={() => setOpen(!open)} className={`text-sm duration-200 cursor-pointer ${open ? "rotate-90" : ""}`} />
        )}
      </div>
      {open && pages && (
        <div className="flex">
