...
export const EditGraph = ({ comp, setComp }: { comp: GraphCompProps; setComp: SetComp }) => {
  return (
    <EditSection title="Graph">
        ...
        <div className="col-span-2 flex flex-col space-y-1 items-center">
          {comp.values.map((value, i) => (
            <div key={i} className="flex space-x-1 w-full">
              ...
              />
              <button
                onClick={() => setComp({ ...comp, values: comp.values.filter((v, j) => j !== i) })}
              >
                X
              </button>
            </div>
      ...
      <SelectInput
        ...
        options={Object.entries(GraphTypes).map(([value, label]) => ({ value, label }))}
      />
      <ColorInput
        label="Color"
        value={comp.color}
      ...
      />
      <NumberInput label="MAX" value={comp.max} onChange={(max) => setComp({ ...comp, max })} />
      <NumberInput label="MIN" value={comp.min} onChange={(min) => setComp({ ...comp, min })} />
      {comp.graphType === "bar" && (
        <>
