...
export default async function Progress(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === "GET") result = await getProgress({ renderId }, { req, res });
...
export const getProgress = async (
    ...
    const { overallProgress, costs, outputFile, fatalErrorEncountered, done } = await getRenderProgress({ bucketName, functionName, region, renderId })
    return {
