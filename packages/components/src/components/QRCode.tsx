...
export const QRCode = ({ text, color, bg, style, className }: QRCodeProps & StyleAndClass) => {
  const fgColor = useColor(color);
  const bgColor = useColor(bg);
  return <ReactQRCode className={className} style={{ width: "100%", height: "100%", ...style }} value={text} fgColor={fgColor} bgColor={bgColor} />;
};
