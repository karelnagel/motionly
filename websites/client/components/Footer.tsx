  ...
  return (
    <div className="flex flex-col relative w-full space-y-7 py-7 px-2 md:px-6 text-center md:text-left">
        ...
        <div className="space-x-2 flex">
          {socials.map((social, i) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-base-300 rounded-full aspect-square !p-2"
            >
          ...
          <div key={i} className="space-y-2">
            ...
            <div className="flex flex-col space-y-1 items-center md:items-start">
              {nav.items.map((item, i) => (
                <a
                  key={i}
                  className="text-list-subtitle text-[16px] font-normal hover:text-primary duration-200"
                  href={item.href}
                >
                  {item.name}
                </a>
      ...
      <div className="font-normal text-sm text-list-subtitle flex flex-col-reverse md:flex-row justify-between">
        <p>
          Copyright © {new Date().getFullYear()} Motionly. All rights reserved
        </p>
